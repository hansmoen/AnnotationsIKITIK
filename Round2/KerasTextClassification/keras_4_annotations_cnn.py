...
from keras.layers import Dense, Dropout, Activation, Embedding, Convolution1D, MaxPooling1D, Flatten, Input, merge, SimpleRNN, GRU
...
if __name__ == "__main__":
    ####################################%%%%%%%%%%%%%%%%%%%%%
    parser = argparse.ArgumentParser(description='keras_4_annotations_cnn.py')
    """
    ## LOCAL TESTING ###################%%%%%%%%%%%%%%%%%%%%%
    ...
    word_merged = merge([word_encoded, word_c2, word_c3, word_c4], mode='concat')

    # LEMMA ------------------------------------
    lemma_inputs = Input(shape=(X_used_row_len,))
    lemma_embedding = Embedding(input_dim=X_lemma_max_value + 1, output_dim=lemma_embeddings_dim, input_length=X_used_row_len, weights=lemma_weights, dropout=0.2, trainable=True, mask_zero=False)(lemma_inputs)

    lemma_encoded = Convolution1D(50, 3, border_mode='valid')(lemma_embedding)
    lemma_encoded = MaxPooling1D(X_used_row_len - 2)(lemma_encoded)
    lemma_encoded = Flatten()(lemma_encoded)

    lemma_c2 = Convolution1D(50, 4, border_mode='valid')(lemma_embedding)
    lemma_c2 = MaxPooling1D(X_used_row_len - 3)(lemma_c2)
    lemma_c2 = Flatten()(lemma_c2)

    lemma_c3 = Convolution1D(50, 5, border_mode='valid')(lemma_embedding)
    lemma_c3 = MaxPooling1D(X_used_row_len - 4)(lemma_c3)
    lemma_c3 = Flatten()(lemma_c3)

    lemma_c4 = Convolution1D(50, 2, border_mode='valid')(lemma_embedding)
    lemma_c4 = MaxPooling1D(X_used_row_len - 1)(lemma_c4)
    lemma_c4 = Flatten()(lemma_c4)

    lemma_merged = merge([lemma_encoded, lemma_c2, lemma_c3, lemma_c4], mode='concat')


    # POS --------------------------------------
    pos_inputs = Input(shape=(X_used_row_len,))
    pos_embedding = Embedding(input_dim=X_pos_max_value + 1, output_dim=pos_embeddings_dim, input_length=X_used_row_len, weights=pos_weights, dropout=0.2, trainable=True, mask_zero=False)(pos_inputs)

    pos_encoded = Convolution1D(50, 3, border_mode='valid')(pos_embedding)
    pos_encoded = MaxPooling1D(X_used_row_len - 2)(pos_encoded)
    pos_encoded = Flatten()(pos_encoded)

    pos_c2 = Convolution1D(50, 4, border_mode='valid')(pos_embedding)
    pos_c2 = MaxPooling1D(X_used_row_len - 3)(pos_c2)
    pos_c2 = Flatten()(pos_c2)

    pos_c3 = Convolution1D(50, 5, border_mode='valid')(pos_embedding)
    pos_c3 = MaxPooling1D(X_used_row_len - 4)(pos_c3)
    pos_c3 = Flatten()(pos_c3)

    pos_c4 = Convolution1D(50, 2, border_mode='valid')(pos_embedding)
    pos_c4 = MaxPooling1D(X_used_row_len - 1)(pos_c4)
    pos_c4 = Flatten()(pos_c4)

    pos_merged = merge([pos_encoded, pos_c2, pos_c3, pos_c4], mode='concat')


    # Combined ---------------------------------
    combined_merged = merge([word_merged, lemma_merged, pos_merged], mode='concat')

    predictions = Dense(output_dim=y_max_value, activation='sigmoid')(combined_merged)

    final_model = Model(input=[word_inputs, lemma_inputs, pos_inputs], output=[predictions])
